...
module Multi
  module Tenant
    ...
    def self.postgresql_adapter(config)
      ...
      adapter = Adapters::PostgresqlSchemaAdapter if Multi.use_schemas
      adapter = Adapters::PostgresqlSchemaFromSqlAdapter if Multi.use_sql && Multi.use_schemas
      adapter.new(config)
    end
  end
  ...
  module Adapters
    ...
    private
      ...
      def preserving_search_path
        ...
        yield
        Multi.connection.execute("set search_path = #{search_path}")
      end
      ...
      def swap_schema_qualifier(sql)
        sql.gsub(/#{default_tenant}\.\w*/) do |match|
          if Multi.pg_excluded_names.any? { |name| match.include? name }
            match
          else
