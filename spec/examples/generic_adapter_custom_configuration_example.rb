...
shared_examples_for "a generic apartment adapter able to handle custom configuration" do
  ...
  before do
    Multi.tenant_names = custom_tenant_names
    Multi.with_multi_server_setup = true
  end

  after do
    Multi.with_multi_server_setup = false
  end
  ...
  context "database key taken from specific config" do
    ...
    describe "#create" do
      it "should establish_connection with the separate connection with expected args" do
        expect(Multi::Adapters::AbstractAdapter::SeparateDbConnectionHandler).to receive(:establish_connection).with(expected_args).and_call_original

        # because we dont have another server to connect to it errors
        # what matters is establish_connection receives proper args
        expect { subject.create(custom_tenant_name) }.to raise_error(Multi::TenantExists)
      end
    end
    ...
    describe "#drop" do
      it "should establish_connection with the separate connection with expected args" do
        ...
        # because we dont have another server to connect to it errors
        # what matters is establish_connection receives proper args
        expect { subject.drop(custom_tenant_name) }.to raise_error(Multi::TenantNotFound)
      end
    end
  end
  ...
  context "database key from tenant name" do
    ...
    describe "#switch!" do
      ...
      it "should connect to new db" do
        expect(Multi).to receive(:establish_connection) do |args|
        ...
        expect { subject.switch!(custom_tenant_name) }.to raise_error(Multi::TenantNotFound)
      end
    end
  end
