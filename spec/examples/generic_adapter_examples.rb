...
shared_examples_for "a generic multi adapter" do
  ...
  before {
    Multi.prepend_environment = false
    Multi.append_environment = false
  }
  ...
  describe "#init" do
    it "should not retain a connection after railtie" do
      ...
      unless Rails::VERSION::MAJOR < 4
        ...
        Multi::Railtie.config.to_prepare_blocks.map(&:call)

        num_available_connections = Multi.connection_class.connection_pool
    ...
    it "should raise error when the schema.rb is missing unless Multi.use_sql is set to true" do
      ...
      begin
        Dir.mktmpdir do |tmpdir|
          Multi.database_schema_file = "#{tmpdir}/schema.rb"
          expect {
            subject.create(db1)
          }.to raise_error(Multi::FileNotFound)
        end
      ensure
        Multi.remove_instance_variable(:@database_schema_file)
      end
    end
  end
